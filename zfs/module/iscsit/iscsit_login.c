...
//insert a login_event_ctx_t to &ict->ict_login_sm.
//iter ict->ict_login_sm, for every login_event_ctx_t,
//exec login_sm_event_dispatch
void
...
//use pdu->isp_hdr to initialize ict->ict_login_sm.icl_login_resp_tmpl for sending
static void
login_sm_handle_initial_login(iscsit_conn_t *ict, idm_pdu_t *pdu)
{
...
//check InitiatorName,SessionType,TargetName of lsm->icl_request_nvlist.
//remove those to lsm->icl_negotiated_values.
//set ict->ict_ic initiator_name,targetname,tsih,isid
static idm_status_t
login_sm_validate_initial_parameters(iscsit_conn_t *ict)
{
	int		nvrc;
