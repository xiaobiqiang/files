...
typedef enum {
	SS_UNDEFINED = 0,
	SS_Q1_FREE,			//created
	SS_Q2_ACTIVE,		//svc_enabled already during login
	SS_Q3_LOGGED_IN,
	SS_Q4_FAILED,
	SS_Q5_CONTINUE,
	SS_Q6_DONE,
...
typedef struct {
	...
	/*
	 *SS_Q1_FREE --sess_create--> SS_Q2_ACTIVE
	 *		iscsit_sess_bind_conn
...
typedef enum {
	ILS_UNDEFINED = 0,
	//handler: login_sm_init
	ILS_LOGIN_INIT,
	ILS_LOGIN_WAITING,	/* Waiting for more login PDU's */
	//handler: login_sm_process_request
...
typedef enum {
	ILE_UNDEFINED = 0,
	ILE_LOGIN_RCV,			//idm recv LOGIN_CMD, then transport to iscsit by iscsit_rx_pdu
...
typedef struct {
	uint32_t		op_initial_params_set:1,	//true when login validated success
				op_discovery_session:1,			//trye if this is to discovery targets
				op_initial_r2t:1,
				op_immed_data:1,
...
typedef struct {
	...
	int			icl_auth_pass;
	/* login_event_ctx_t list */
	list_t			icl_login_events;
	list_t			icl_pdu_list;
	//if to discovery targets, it's ISCSI_UNSPEC_TSIH
	uint16_t		icl_tsih;
