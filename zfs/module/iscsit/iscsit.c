	...
	case IDM_STATUS_ABORTED:
		...
		if (itask->it_stmf_abort) {
			...
			stmf_abort(STMF_QUEUE_TASK_ABORT, itask->it_stmf_task,
			    STMF_ABORTED, NULL, ISCSIT_TASK_ABORTED);
			return;
		}
		/*NOTREACHED*/
	default:
...
//kmem_zalloc a iscsit_conn_t(named ict) and init ict.
//set ic->ic_handle to ict
static idm_status_t
iscsit_conn_accept(idm_conn_t *ic)
{
	iscsit_conn_t *ict;
...
iscsit_op_scsi_task_mgmt(iscsit_conn_t *ict, idm_pdu_t *rx_pdu)
{
	...
	case ISCSI_TM_FUNC_ABORT_TASK:
		...
		if (itask == NULL) {
			 ...
			 */
			stmf_abort(STMF_QUEUE_TASK_ABORT, itask->it_stmf_task,
			    STMF_ABORTED, NULL, ISCSIT_OP_SCSI_TASK_MGMT);
