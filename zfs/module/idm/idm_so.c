...
idm_transport_ops_t idm_so_transport_ops = {
	...
	//get local and peer sockname, and 
	//start so_conn->ic_tx_thread,ic_rx_thread
 ...
 * occurs.
 *
 * Returns 0 for success, otherwise returns -EPIPE for broken socket.
 */
int
idm_sosendto(struct socket *so, void *buff, size_t len,
    struct sockaddr *name, int namelen)
{
	...
	if ((sent = kernel_sendmsg(so, &msg, &iov[0], 1, len)) != len) {
		return -EPIPE;
	}
 ...
 */
int
idm_iov_sosend(struct socket *so, iovec_t *iop, int iovlen, size_t total_len)
{
	struct msghdr		msg;
	int 			sent = 0;
	...
	if ((sent = kernel_sendmsg(so, &msg, iop, 
			(size_t)iovlen, total_len)) != total_len) {
		/* broken socket */
		return -EPIPE;
	}
 ...
 * occurs.
 *
 * Returns 0 for success, -EPIPE for broken socket, -EIO for io error and 
 * <0 for other errors.
 */
int
idm_iov_sorecv(struct socket *so, iovec_t *iop, int iovlen, size_t total_len)
{
	struct msghdr		msg;
	int			error;
	int			recv;
	...
	if ((recv = kernel_recvmsg(so, &msg, iop, 
			iovlen, total_len, flags)) != total_len) {
		/* broken pipe */
		if (!recv)
			return -EPIPE;
		/* IO error */
...
idm_set_ini_preconnect_options(idm_so_conn_t *sc, boolean_t boot_conn)
{
	...
	int	abort = 30000;
/*
	(void) kernel_setsockopt(sc->ic_so, IPPROTO_TCP,
...
idm_set_ini_postconnect_options(idm_so_conn_t *sc)
{
	...
	(void) kernel_setsockopt(sc->ic_so, IPPROTO_TCP, TCP_NODELAY,
	    (char *)&on, sizeof (int));
	(void) kernel_setsockopt(sc->ic_so, SOL_SOCKET, SO_RCVBUF,
	    (char *)&rcvbuf, sizeof (int));
	(void) kernel_setsockopt(sc->ic_so, SOL_SOCKET, SO_SNDBUF,
...
idm_set_tgt_connect_options(struct socket *ks)
{
	...
	/* Set connect options */
	(void) kernel_setsockopt(ks, SOL_SOCKET, SO_RCVBUF,
	    (char *)&rcvbuf, sizeof (int));
	(void) kernel_setsockopt(ks, SOL_SOCKET, SO_SNDBUF,
	    (char *)&sndbuf, sizeof (int));
	(void) kernel_setsockopt(ks, IPPROTO_TCP, TCP_NODELAY,
	    (char *)&on, sizeof (on));
}
...
idm_so_ini_conn_create(idm_conn_req_t *cr, idm_conn_t *ic)
{
	...
	if (cr->cr_bound) {
		if (kernel_bind(so, &cr->cr_bound_addr.sin,
		    SIZEOF_SOCKADDR(&cr->cr_bound_addr.sin)) != 0) {
			idm_sodestroy(so);
			return (IDM_STATUS_FAIL);
		}
	}
...
//		idm_so_socket_set_nonblock(node);
		do {
			rc = kernel_connect(so_conn->ic_so,
...
//			idm_so_socket_set_block(node);
		...
		rc = kernel_connect(so_conn->ic_so, &ic->ic_ini_dst_addr.sin,
		    (SIZEOF_SOCKADDR(&ic->ic_ini_dst_addr.sin)), 0);
	}
...
idm_status_t
idm_so_tgt_conn_create(idm_conn_t *ic, struct socket *new_so)
{
	idm_status_t	idmrc;
...
static idm_status_t
idm_so_conn_create_common(idm_conn_t *ic, struct socket *new_so)
{
	idm_so_conn_t	*so_conn;
...
idm_so_conn_connect_common(idm_conn_t *ic)
{
	...
	(void) kernel_getsockname(so_conn->ic_so, (struct sockaddr *)&t_addr,
	    &t_addrlen);
	bcopy(&t_addr, &ic->ic_laddr, t_addrlen);
	(void) kernel_getpeername(so_conn->ic_so, (struct sockaddr *)&t_addr,
	    &t_addrlen);
...
//create a socket, bind and listen. then create a thread to accept
//idm_so_svc_port_watcher do this.
static idm_status_t
...
/*		(void) ksocket_setsockopt(so_svc->is_so, SOL_SOCKET,
		...
		if (kernel_bind(so_svc->is_so, (struct sockaddr *)&sin6_ip,
		    sizeof (sin6_ip)) != 0) {
			mutex_exit(&is->is_mutex);
	...
	if (kernel_listen(so_svc->is_so, 5) != 0) {
		mutex_exit(&is->is_mutex);
...
//	so_svc->is_thread_did = so_svc->is_thread->t_did;
	...
	while (so_svc->is_thread_running) {
		...
		if ((rc = kernel_accept(so_svc->is_so,
		    &new_so, 0)) != 0) {
			mutex_enter(&svc->is_mutex);
			if (rc == -ECONNABORTED)
				continue;
...
idm_sorecvdata(idm_conn_t *ic, idm_pdu_t *pdu)
{
	...
	if ((ic->ic_conn_flags & IDM_CONN_DATA_DIGEST) != 0) {
		...
		iov++;
		while (iov->iov_base != (caddr_t)&data_digest_crc) {
