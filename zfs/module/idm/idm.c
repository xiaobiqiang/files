...
idm_transport_t idm_transport_list[] = {
	...
	NULL,				/* device path */
	NULL,				/* LDI handle */
	NULL,				/* transport ops:idm_so_transport_ops */
	NULL}				/* transport caps */
...
//在audit_buf->sab_records中记录一条sm_audit_record_t
void
...
idm_fini(void)
{
	...
	if ((rc = _idm_fini()) != 0) {
		cmn_err(CE_NOTE, 
			"_idm_fini failed.error:%d", rc);
		return ;
	}
}

module_init(idm_init);
module_exit(idm_fini);
MODULE_LICENSE("GPL");
