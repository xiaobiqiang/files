...
class Server:
    ...
    def update(self,ip,state):
        ...
        if ip == '127.0.0.1':
            ...
            ret,hostname = subprocess.getstatusoutput("grep "+ip+" /etc/clumgt.config|awk '{printf $1}'")
    ...
    def node_update_thread(self):
        ...
        while self.updateThread:
            ...
            ret,flag = subprocess.getstatusoutput("cat "+self.updateFile)
            flag = flag.strip()
    ...
    def node_init(self):
        if os.path.isfile(self.rdbFile):
            self.rdb_get()
            ret,self.rdDir = subprocess.getstatusoutput("ls "+self.rdFileDir+"|egrep -v unpack|awk '{printf $1}'")
    ...
    def server_process(self):
        ...
        ret,self.myself = subprocess.getstatusoutput("hostname|awk '{printf $1}'")
        self.node_init()
...
if __name__ == '__main__':
    fileName = sys.argv[0]
    ret,isRun = subprocess.getstatusoutput("ps -ef |grep "+fileName+"|egrep -v 'grep|sh -c|cm_multi_exec'|wc -l")
