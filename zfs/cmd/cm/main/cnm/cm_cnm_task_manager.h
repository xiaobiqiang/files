...
typedef struct
{
    ...
    cm_time_t start;
    cm_time_t end;
    uint8 desc[CM_STRING_256];
}cm_cnm_localtask_info_t;

extern sint32 cm_cnm_localtask_init(void);

extern sint32 cm_cnm_localtask_decode(
    const cm_omi_obj_t ObjParam, void**ppDecodeParam);
    
extern cm_omi_obj_t cm_cnm_localtask_encode(
    const void *pDecodeParam, void *pAckData, uint32 AckLen);

extern sint32 cm_cnm_localtask_getbatch(
    const void *pDecodeParam,void **ppAckData, uint32 *pAckLen);

extern sint32 cm_cnm_localtask_get(
    const void *pDecodeParam,void **ppAckData, uint32 *pAckLen);  
  
extern sint32 cm_cnm_localtask_count(
    const void *pDecodeParam,void **ppAckData, uint32 *pAckLen);
    
extern sint32 cm_cnm_localtask_delete(
    const void *pDecodeParam,void **ppAckData, uint32 *pAckLen);

extern sint32 cm_cnm_localtask_local_get
(void *param, uint32 len,
 uint64 offset, uint32 total,
 void **ppAck, uint32 *pAckLen);
extern sint32 cm_cnm_localtask_local_delete
(void *param, uint32 len,
 uint64 offset, uint32 total,
 void **ppAck, uint32 *pAckLen);
