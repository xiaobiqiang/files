...
st_zfs_callback(zfs_handle_t *zhp, void *data)
{
	...
	if (cb->cb_flags & ST_ZFS_ITER_RECURSE &&
		...
		if ((zfs_get_type(zhp) != ZFS_TYPE_SNAPSHOT) && include_snaps)
			(void) zfs_iter_snapshots(zhp, st_zfs_callback, data);
		cb->cb_depth--;
	}
...
int
st_zfs_for_each(int flags, zfs_type_t types,
    st_zfs_sort_column_t *sortcol, zprop_list_t **proplist, int limit,
    st_zfs_iter_cb callback,void *data, boolean_t flag)
{
	...
	ret = zfs_iter_root(g_zfs, st_zfs_callback, &cbp);
	...
	for (node = uu_avl_first(cbp.cb_avl); node != NULL;
		...
		ret |= callback(node->zn_handle, node->zn_depth, data, flag);
