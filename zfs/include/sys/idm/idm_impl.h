...
typedef struct idm_svc_s {
	...
	void			*is_so_svc; /* TCP:idm_so_svc_t kmem_zalloc in idm_so_tgt_svc_create */
	void			*is_iser_svc;	/* IB:? */
...
typedef struct idm_conn_s {
	list_node_t		ic_list_node;	 /* append to idm.idm_tgt_conn_list */
	void			*ic_handle;
	...
	/*
	 * CS_S1_FREE --accept--> CS_S3_XPT_UP --LOGIN_CMD--> CS_S4_IN_LOGIN -->
	 * 
	 */
	...
	void			*ic_transport_private;	/* idm_so_conn_t */
...
/* <struct idm_pdu_s>+<iscsi_hdr_t>+<ahs> */
typedef struct idm_pdu_s {
	...
	iscsi_hdr_t	*isp_hdr;
	/* sizeof (iscsi_hdr_t) + (isp_hdr->hlength * sizeof (uint32_t)) */
	uint_t		isp_hdrlen;
	uint8_t		*isp_data;
	/* n2h24(isp_hdr->dlength) */
	uint_t		isp_datalen;
	 ...
	 */
	void		*isp_private;
	/* idm_sorx_addl_pdu_cb for free pdu */
	//login: login_resp_complete_cb
...
#define	IDM_PDU_ALLOC		0x00000001	/* for ini */
#define	IDM_PDU_ADDL_HDR	0x00000002	/* bhs->hlength > IDM_SORX_CACHE_AHSLEN,pdu->isp_hdr used kmem_alloc.  */
#define	IDM_PDU_ADDL_DATA	0x00000004	/* like above but for data */
